...
public class ShootCommandGroup extends SequentialCommandGroup {
    ...
    public ShootCommandGroup(ShooterSubsystem shooter, SpindexerSubsystem spindexer, TransferArmSubsystem transferArm, TransferWheelSubsystem transferWheel, IntSupplier ballColor, AprilTagSubsystem atag, int ballNum, IntSupplier offset) {
//        telemetry.addData("hello", 1);
        addCommands(
                ...
                new InstantCommand(() -> Log.i("SHOOT COMMAND", "starting shoot command" + (ballNum + offset.getAsInt())%3 + " is ball color")),
                new LogKittenCommand(Log.ASSERT, "SHOOT COMMAND", "starting shoot command" + (ballNum + offset.getAsInt())%3 + " is ball color"),
                new ShootWithDistCommand(shooter, atag).alongWith(new SpindexOutCommand(spindexer, ballColor.getAsInt(), ballNum)),
